...
if (-not (Test-Path -LiteralPath $publishFolder)) {
    Write-Host "1/6 Cloning the verified GitHub repository..." -ForegroundColor Cyan
    ...
    Write-Host "1/6 Using the existing verified clone..." -ForegroundColor Cyan
}
...
Write-Host "2/6 Getting the latest GitHub version..." -ForegroundColor Cyan
...
if ($WorkbookPath) {
    Write-Host "3/6 Extracting data and photos from Excel..." -ForegroundColor Cyan
    ...
    & $pythonPath (Join-Path $publishFolder "scripts\extract_catalog.py") $WorkbookPath
    if ($LASTEXITCODE -ne 0) {
        throw "The Excel export failed. Nothing was sent to GitHub."
    }
} else {
    Write-Host "3/6 Using the current catalogue files..." -ForegroundColor Cyan
}
...
Write-Host "4/6 Checking the catalogue update..." -ForegroundColor Cyan
